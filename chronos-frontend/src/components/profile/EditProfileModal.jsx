...
export default function EditProfileModal({
  ...
  const currentAvatar = useMemo(() => {
    if (!me) return null;
    return preview || me.avatarUrl || (me.avatar ? absUrl(me.avatar) : null);
  }, [me, preview]);
  ...
  const canSubmit = useMemo(() => {
    ...
    const nameValid = !wantsName || USER_RE.test(username.trim().toLowerCase());
  ...
  async function submit() {
    ...
    try {
      ...
      if (nameChanged) fd.append("username", username.trim().toLowerCase());
  ...
  async function doDelete() {
    if (!askDelete) { setAskDelete(true); return; }
    setLoading(true);
    setErr("");
    try {
  ...
  return (
    <div className="ep-overlay" onMouseDown={onOverlay} onKeyDown={onKey} role="dialog" aria-modal="true">
      <div className="ep-modal" onMouseDown={(e)=>e.stopPropagation()}>
        <div className="ep-grid">
          <div className="ep-left">
            ...
            <p className="ep-description">Update your profile information</p>
            ...
            {err && <div className="ep-error" role="alert">{err}</div>}

            <div className="ep-actions">
              <button
                className={`ep-btn1 ep-btn--danger${askDelete ? " is-ask" : ""}`}
                onClick={doDelete}
                disabled={loading}
                title="Delete account"
              >
                {askDelete ? "Confirm delete" : "Delete account"}
              </button>
          ...
          <div className="ep-right">
            <div className="ep-avatarWrap">
              ...
              <label className="ep-uploadBtn" title="Upload avatar">
