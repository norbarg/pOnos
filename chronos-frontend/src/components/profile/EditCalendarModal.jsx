...
export default function EditCalendarModal({ open, calendar, onClose, onSaved }) {
  const cal = calendar || {};
  ...
  const canSubmit = useMemo(() => {
    const nm = name.trim();
    const ds = desc.trim();
    return changed && nm.length >= NAME_MIN && ds.length >= DESC_MIN && !saving;
  ...
  async function submit() {
    ...
    try {
      ...
      onSaved?.(data?.calendar || { ...cal, name: nm, description: ds, color });
      onClose?.();
    } catch (err) {
      ...
      if (code === 409 || msg === "duplicate-name") {
        ...
        setError(msg === "name-required" ? "Title is required." : "Description is required.");
      } else {
    ...
    >
      <div className="nc-modal" onMouseDown={(e) => e.stopPropagation()}>
        ...
        <div className="nc-actions">                                      
          <button className="nc-btn nc-btn--ghost" onClick={onClose} disabled={saving}>
            Cancel
          </button>
