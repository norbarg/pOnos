// chronos-frontend/src/pages/AcceptInvite.jsx
    ...
    const run = async () => {
      ...
      try {
        ...
        if (data.kind === 'calendar') {
          ...
          navigate(`/calendars?cal=${encodeURIComponent(data.calendarId)}`, {
            replace: true,
          });
          return;
        }
      ...
      <section className="acceptPage">
        <div className="acceptCard">
          ...
          <div className="acceptCard__bottom">
            ...
            <div className="acceptCard__list">
              {calendars.map((c) => (
                <label
                  ...
                  className={
                    'acceptCal' +
                    (selectedCal === c.id ? ' acceptCal--selected' : '')
                  }
                  onClick={() => setSelectedCal(c.id)}
                >
                  <span className="acceptCal__radio" />
                  <span className="acceptCal__label">{c.name}</span>
            ...
            {error && (
              <p className="acceptCard__error">{error}</p>
            )}
          </div>
        </div>
      </section>
