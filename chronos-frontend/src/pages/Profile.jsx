...
const CAT_ICONS = { task: icTask, reminder: icReminder, arrangement: icArrangement };
...
function pickTwoUpcomingOrLast(all, now = new Date()) {
  ...
  if (future.length) return { preview: future.slice(0, 2), more: future.length > 2 };
  const past = list
    .filter(e => e?.start && new Date(e.start) < now)
...
export default function Profile() {
  ...
  const me = useMemo(() => {
    ...
    const name = user.name || user.username || (user.email ? user.email.split('@')[0] : '');
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const holidays = raw.find(c => c.isSystem && c.systemType === 'holidays');
        const visible = raw.filter(c => !(c.isSystem && c.systemType === 'holidays'));
...
const withMembers = await Promise.all(
  visible.map(async (cal) => {
    try {
      const { data: mdata } = await api.get(`/calendars/${cal.id}/members`);
      const list = Array.isArray(mdata?.members) ? mdata.members : [];

      const names = list
        .map(m => m?.name || (m?.email ? m.email.split('@')[0] : 'user'))
        .filter(Boolean);
      ...
      const ownerName =
        mdata?.owner?.name ||
        (mdata?.owner?.email ? mdata.owner.email.split('@')[0] : 'user');
      ...
      return {
          ...
          withMembers.map(async (cal) => {
            ...
            try {
              const { data: edata } = await api.get(
                ...
                { params: { from: from.toISOString(), to: to.toISOString(), expand: 1 } }
              );
              evs = Array.isArray(edata?.events) ? edata.events : [];
            } catch {}
            if (!evs.length) {
              try {
                const { data: allData } = await api.get(`/calendars/${cal.id}/events`);
                evs = Array.isArray(allData?.events) ? allData.events : [];
              } catch {}
            }
            ...
            if (!desc && cal.isMain && cal.hasHolidays && cal.countryCode) {
        ...
        console.warn('calendars load failed:', e?.response?.data || e.message);
        setCalendars([]);
      }
    })();
  }, [dispatch]);
...
function handleUpdated() {
  ...
  function openMenu(id) { setMenuFor(prev => prev === id ? null : id); }
  function closeMenu() { setMenuFor(null); }
  function applyCalendarPatch(upd) {
    if (upd?.__openEdit) { setEditCal(upd); return; }
    setCalendars(prev => prev.map(c => c.id === upd.id ? { ...c, ...upd } : c));
  }
        ...
        <div className="profilePanel">
          <div className="identity-row">
            <div className="identity-left">
              <div className="identity-name">{me?.name || '—'}</div>
              ...
              <div className="identity-actions">
                <button className="btn btn--small" data-open="edit-profile" onClick={() => setOpenEdit(true)}>Edit profile</button>
              </div>
            </div>
            ...
            <div className="identity-right">
              {me?.avatarUrl && !avatarError ? (
                ...
                <div className="identity-avatar identity-avatar--fallback" aria-label="avatar initial">
          ...
          <div className="panelHeader">
            ...
            <button className="btn1" data-open="new-calendar" onClick={() => setOpenNew(true)}
              >New calendar</button>
          </div>
          ...
          <div className="calendar-grid">
            {calendars.map((cal) => {
              ...
              return (
                ...
                <div
  key={cal.id}
  className={`calendar-card ${menuFor === cal.id ? 'card--menu-open' : ''} ${!cal.active ? 'is-inactive' : ''}`} // NEW
>
                  <div
                    className="calendar-header"                          
                    style={{ background: calColor, borderColor: calBorder }}
                  >
                    <h3 className="calendar-header__title oneLine">
                      <Link to={`/calendar/${cal.id}`}>{cal.name}</Link>
                    </h3>
                  </div>
                  ...
                  <div className="calendar-body">                
                    {!cal.isMain && (                                  
                      <img
                        onClick={(e)=>{ e.stopPropagation(); openMenu(cal.id); }}
                    ...
                    {cal.description && (
                      <p className="calendar-desc oneLine">{cal.description}</p>
                    )}
                    ...
                    <div className="calendar-metaBlock">
                      ...
                      {cal.owner === me?.id
                      ? 'you'
                      : (cal._ownerName || cal.owner)}
                      </div>
                      {!cal.isMain && (
                        <div className="calendar-metaRow oneLine">
                          members:{' '}
                          {cal._membersPreview?.length
                            ? cal._membersPreview.join(', ') + (cal._membersHasMore ? ' …' : '')
                            : '—'}
                        </div>
                      )}
                    </div>
                    ...
                    <div className="eventPreviewList">
                      {(cal._eventsPreview?.slice(0,2) || []).length ? (
                        cal._eventsPreview.slice(0,2).map((ev) => {
                          const color = ev?.categoryInfo?.color || '#151726';
                          const bg = hexToRgba(color, 1);
                          const bd = hexToRgba(color, 1);
                          const icon = iconForEvent(ev);

                          return (
                            <div
                              key={ev.id || ev._id}
                              className={`eventChip${icon ? '' : ' eventChip--noIcon'}`}
                              style={{ background: bg, borderColor: bd }}
                              title={fmtEventWhen(ev.start)}
                            >
                              {icon && <img src={icon} alt="category" className="eventChip__icon" />}
                              <span className="eventChip__label">{ev.title}</span>
                            </div>
                          );
                        })
                      ) : (
                        <p className="muted1" style={{margin:'0'}}>no events</p>
                      )}
                      {cal._eventsMore && <span className="eventMore">+ more</span>}
                    </div>
                  </div>
                </div>
              );
      ...
      <EditCalendarModal
        ...
        onSaved={(updated)=>{ setEditCal(null); setCalendars(prev => prev.map(c => c.id === updated.id ? { ...c, ...updated } : c)); }}
      />
    </>
  );
}
